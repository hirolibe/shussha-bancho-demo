...
### 4.1 フレームワーク
- **Expo Dev Client + React Native + TypeScript**（既存『アプリ開発案.md §3』に準拠）
...
### 4.3 ハードウェア
- サンワサプライ MM-BLEBC3（既存計画書の選定そのまま）
...
| iOS でアプリ完全終了状態 | 検知不能 | Region Monitoring は OS が起こすが、swipe kill 後は不可（既存計画書 §3.2 で承知の制約） |
...
- [ ] iOS / Android の **どちらか一方** で動作確認済み（両方は Phase 2 までに）
...
1. ビーコン1セット購入（既存計画書 §13 と共通、約 ¥14,800）
2. Slack App を社内ワークスペースに作成、`Install to Workspace` で `xoxp-` トークン取得
3. `:kinmu:` 絵文字をワークスペースに登録
4. Expo プロジェクト作成（`npx create-expo-app`、Dev Client 構成）
5. `react-native-beacon-radar` 動作確認（フォアグラウンド検知）
6. Slack `users.profile.set` 直接呼出 → ステータス変更動作確認
